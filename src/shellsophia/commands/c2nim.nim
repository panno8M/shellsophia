...
import "$nim"/compiler/[ast, parser, renderer, idents, options, lineinfos]

type C2NimArgs* = object
  `in`*: seq[string]
  `out`*: string
  preprocess*: proc(contents: string): string
  postprocess*: proc(contents: PNode): Pnode
  strict*: bool
  cpp*: bool
  dynlib*: string
  header*: string
...
proc c2nim*(shell: ShellEnv; args: C2NimArgs): ShellEnv {.discardable.} =
  var a = args.in
  if args.preprocess != nil:
    for i, ain in args.in:
      var file: File
      (file, a[i]) = createTempFile("", ain.extractFilename)
      file.write args.preprocess(readFile(ain))
      close file
